...
const doctorNavItems = [
  ...
  { path: '/doctor/schedule', label: 'Schedule', icon: 'calendar_month' },
  { path: '/doctor/notes', label: 'Clinical Notes', icon: 'clinical_notes' },
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  ...
  const { isDoctor, hospital, staff, logout } = useAuth();
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  const navItems = isDoctor ? doctorNavItems : adminNavItems;
  const portalLabel = isDoctor ? 'Doctor Portal' : 'Admin Portal';

  // Identity card info
  const displayName = isDoctor
    ? staff?.full_name ?? 'Doctor'
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center gap-3 px-6 py-6 border-b border-white/5">
            ...
            <div>
              ...
              <span className={`text-[10px] uppercase tracking-widest font-semibold ${isDarkMode ? 'text-primary/60' : 'text-green-600/60'}`}>
                {portalLabel}
              </span>
            </div>
          </div>
          ...
          <nav className="flex-1 space-y-1 px-4 py-6 overflow-y-auto">
            {navItems.map(item => {
              const isActive = location.pathname === item.path
                || (item.path !== '/' && item.path !== '/doctor' && location.pathname.startsWith(item.path));
              return (
                <button
                  key={item.path}
                  onClick={() => {
            ...
            <div className={`rounded-xl p-4 mb-3 border ${isDarkMode ? 'bg-surface-darker/50 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="overflow-hidden">
                  <p className={`truncate text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{displayName}</p>
                  <p className="truncate text-xs text-slate-500">{displaySub}</p>
                </div>
              </div>
              {isDoctor && staff?.on_duty !== undefined && (
                <div className="mt-2 flex items-center gap-1.5">
                  <span className={`h-2 w-2 rounded-full ${staff.on_duty ? 'bg-green-500' : 'bg-slate-400'}`} />
                  <span className="text-[11px] text-muted-foreground">{staff.on_duty ? 'On Duty' : 'Off Duty'}</span>
                </div>
              )}
            </div>
