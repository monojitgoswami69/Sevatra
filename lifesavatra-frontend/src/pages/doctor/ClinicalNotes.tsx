...
import { getClinicalNotes, addClinicalNote, getDoctorPatients } from '../../services/doctorService';
import type { ClinicalNote } from '../../types/dashboard.types';
import type { AdmittedPatient } from '../../services/doctorService';
...
const ClinicalNotes: React.FC = () => {
  ...
  // New note form
  const [notePatientId, setNotePatientId] = useState<string>('');
  ...
  useEffect(() => {
    Promise.all([getClinicalNotes(), getDoctorPatients()])
      .then(([n, p]) => {
        setNotes(n);
        setPatients(p);
        if (p.length > 0) setNotePatientId(String(p[0].patient_id));
      })
  ...
  const filtered = useMemo(() => {
    ...
    if (filterPatient !== 'all') list = list.filter(n => n.patient_id === filterPatient);
    return list;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const patient = patients.find(p => String(p.patient_id) === notePatientId);
    if (!patient) return;
    setSaving(true);
    try {
    ...
    <DashboardLayout>
      <div className="relative z-10 p-6 lg:p-8 space-y-6">
        ...
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-card-foreground tracking-tight">Clinical Notes</h1>
          ...
          <button onClick={() => setShowForm(!showForm)}
            className={`flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all ${
              showForm
                ...
                : 'bg-primary text-green-950 shadow-[0_0_15px_rgba(19,236,19,0.3)] hover:shadow-[0_0_25px_rgba(19,236,19,0.5)]'
            }`}>
        ...
        {/* New note form */}
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-card rounded-2xl border border-primary/20 shadow-lg p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <select value={notePatientId} onChange={e => setNotePatientId(e.target.value)}
                  className="w-full rounded-xl border border-border bg-[var(--input-bg)] px-4 py-2.5 text-[var(--input-text)] text-sm focus:border-primary outline-none transition-all">
                  {patients.map(p => (
            ...
            <div>
              ...
              <textarea rows={4} value={noteText} onChange={e => setNoteText(e.target.value)}
                ...
                className="w-full rounded-xl border border-border bg-[var(--input-bg)] px-4 py-3 text-[var(--input-text)] text-sm placeholder:text-[var(--input-placeholder)] focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all resize-none" />
            </div>
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          className="rounded-xl border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground focus:border-primary outline-none transition-all">
        ...
        {loading ? (
          ...
          <div className="bg-card rounded-2xl border border-border p-12 text-center">
            <span className="material-symbols-outlined text-4xl text-muted-foreground mb-2 block">description</span>
          ...
          <div className="space-y-3">
            {filtered.map(note => {
              ...
              return (
                <div key={note.id} className="bg-card rounded-2xl border border-border shadow-lg p-5 hover:border-border/70 transition-all">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <button onClick={() => navigate(`/patients/${note.patient_id}`)}
                          className="text-card-foreground font-semibold text-sm hover:text-primary transition-colors">
                          {note.patient_name}
                        </button>
                        <p className="text-[10px] text-muted-foreground">{timeAgo(note.created_at)} • {new Date(note.created_at).toLocaleString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true })}</p>
                      </div>
                    </div>
                  ...
                  <p className="text-muted-foreground text-sm leading-relaxed">{note.note}</p>
                </div>
              );
            })}
          </div>
