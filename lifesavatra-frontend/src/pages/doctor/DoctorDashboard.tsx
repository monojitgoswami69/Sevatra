...
import { getDoctorPatients, getSchedule, getClinicalNotes, type AdmittedPatient } from '../../services/doctorService';
...
const conditionBadge = (c: string) => {
  ...
  return map[c.toLowerCase()] ?? 'bg-primary/10 text-muted-foreground border-border';
};
...
const noteTypeColor: Record<string, string> = {
  ...
  prescription: 'bg-purple-500/10 text-purple-500 border-purple-500/20',
  'discharge-summary': 'bg-green-500/10 text-green-500 border-green-500/20',
};
      ...
      <div className="relative z-10 p-6 lg:p-8 space-y-6">
        ...
        <header>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white tracking-tight">
            {greeting}, {(staff?.full_name ?? 'Doctor').replace('Dr. ', '')}
          </h1>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Patients', value: patients.length, icon: 'group', color: 'text-primary', bg: 'bg-primary/10', border: 'border-primary/20' },
            ...
            { label: 'Schedule Left', value: upcomingSlots.length, icon: 'schedule', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
          ].map(s => (
            <div key={s.label} className={`bg-card rounded-2xl p-5 border ${s.border} shadow-lg relative overflow-hidden`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Patient list */}
          <div className="bg-card rounded-2xl border border-border shadow-lg flex flex-col">
            <div className="p-4 border-b border-border flex justify-between items-center">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-primary text-lg">masks</span>
                <h3 className="text-gray-900 dark:text-white font-semibold">My Patients</h3>
              </div>
              <button onClick={() => navigate('/patients')} className="text-primary text-xs font-semibold hover:underline">
                View All →
              </button>
            </div>
            <div className="p-3 flex-1 space-y-2 overflow-y-auto max-h-80">
              {patients.map(p => (
                <button key={p.patient_id}
                  ...
                  className="w-full p-3 rounded-xl bg-muted border border-border flex items-center gap-3 hover:border-primary/30 transition-all text-left group">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-gray-900 dark:text-white font-medium text-sm truncate group-hover:text-primary transition-colors">
                      {p.patient_name}
                    </p>
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    <span className="text-muted-foreground/60 text-[10px]">Score {p.severity_score}/10</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-lg flex flex-col">
            <div className="p-4 border-b border-border flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-gray-900 dark:text-white font-semibold">Today's Schedule</h3>
                <span className="text-[10px] px-2 py-0.5 bg-primary/10 text-primary rounded-full font-semibold border border-primary/20">
                  {completedSlots}/{schedule.length} done
                </span>
              </div>
              <button onClick={() => navigate('/schedule')} className="text-primary text-xs font-semibold hover:underline">
                Full View →
              </button>
            </div>
                ...
                <div key={slot.id}
                  className={`flex items-center gap-3 p-3 rounded-xl transition-all ${
                    slot.status === 'in-progress'
                      ...
                      : 'bg-muted border border-transparent hover:border-border/80'
                  }`}>
                  <span className="text-xs text-muted-foreground w-16 flex-shrink-0 font-mono">{slot.time}</span>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-muted-foreground/60 text-[10px] capitalize">{slot.type}</p>
                  </div>
                  {slot.status === 'in-progress' && (
        ...
        <div className="bg-card rounded-2xl border border-border shadow-lg">
          <div className="p-4 border-b border-border flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-gray-900 dark:text-white font-semibold">Recent Clinical Notes</h3>
            </div>
            <button onClick={() => navigate('/notes')} className="text-primary text-xs font-semibold hover:underline">
              All Notes →
            </button>
          </div>
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            {notes.slice(0, 4).map(n => (
              <div key={n.id} className="p-4 rounded-xl bg-muted border border-border">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-gray-900 dark:text-white font-medium text-sm">{n.patient_name}</span>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${noteTypeColor[n.type]}`}>
                    {n.type}
                  </span>
                </div>
                <p className="text-muted-foreground text-xs line-clamp-2 leading-relaxed">{n.note}</p>
                <p className="text-muted-foreground/50 text-[10px] mt-2">{timeAgo(n.created_at)}</p>
              </div>
            ))}
          </div>
        </div>
