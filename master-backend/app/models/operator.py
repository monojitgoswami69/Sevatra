...
class OperatorRegisterRequest(BaseModel):
    ...
    # Provider fields (optional — only for provider type)
    ...
    license_number: Optional[str] = Field(None, max_length=100, description="Operator or facility license")
...
class AmbulanceCreate(BaseModel):
    vehicle_number: str = Field(..., min_length=1, max_length=20, description="License plate / registration number")
    ...
    # Operational
    service_radius_km: Optional[float] = Field(None, ge=1, le=500, description="Service area radius in km")
    price_per_km: Optional[float] = Field(None, ge=0, description="Price per km in local currency")
