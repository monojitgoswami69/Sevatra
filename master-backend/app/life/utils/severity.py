...
def _bp_score(sbp: Optional[float], dbp: Optional[float]) -> dict:
    ...
    if sbp < 70:
        score += 3
        factors.append("Critical hypotension (SBP < 70)")
    elif sbp < 90:
        score += 2
        factors.append("Hypotension (SBP 70-90)")
    elif sbp < 100:
        score += 1
        factors.append("Mild hypotension (SBP 90-100)")
    elif sbp > 180:
        score += 3
        factors.append("Hypertensive crisis (SBP > 180)")
    elif sbp > 160:
        score += 2
        factors.append("Severe hypertension (SBP 160-180)")
    elif sbp > 140:
        score += 1
        factors.append("Hypertension (SBP 140-160)")

    if dbp and dbp > 0:
        if dbp < 40:
            score += 2
            factors.append("Critical low DBP (< 40)")
        elif dbp < 60:
            score += 1
            factors.append("Low DBP (40-60)")
        elif dbp > 110:
            score += 2
            factors.append("Critical high DBP (> 110)")
        elif dbp > 90:
            score += 1
            factors.append("High DBP (90-110)")
...
def calculate_severity(
    ...
    """Calculate severity score from vital signs.

    Returns dict with: score, condition, wardRecommendation, riskFactors,
    summary, breakdown, percentage, urgency.
    """
    breakdown = [
...
def validate_vitals(
    ...
    """Validate vital values are within plausible ranges."""
    values = {
    ...
    if bp_systolic is not None and bp_diastolic is not None:
        if bp_systolic < bp_diastolic:
            errors.append("Systolic BP should be >= diastolic BP")
