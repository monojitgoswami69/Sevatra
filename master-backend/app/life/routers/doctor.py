...
Fixes:
- Replaced raw ``data: dict`` on schedule creation with ``ScheduleSlotCreate``.
"""
...
from app.life.dependencies import get_current_staff
...
from app.life.services import doctor_service as svc
...
async def create_schedule(
    ...
    db = get_db()
    slot = await svc.create_schedule_slot(
        db, staff["id"], staff["hospital_id"], data.model_dump()
    )
    return slot
...
async def update_schedule(
    ...
    db = get_db()
    result = await svc.update_schedule_status(
        db, schedule_id, staff["id"], body.status
    )
    if not result:
        raise HTTPException(
            status_code=404, detail="Schedule slot not found or not yours"
        )
    return result
...
async def update_profile(
    ...
    db = get_db()
    updated = await svc.update_doctor_profile(
        db, staff["id"], body.model_dump(exclude_unset=True)
    )
    if not updated:
        raise HTTPException(status_code=404, detail="Profile not found")
    return updated
