...
Fixes applied:
- Added RefreshTokenRequest, BedAssignRequest, DutyToggleRequest,
  ScheduleSlotCreate to replace raw `body: dict` parameters.
"""
...
class RefreshTokenRequest(BaseModel):
    """Typed replacement for raw ``body: dict``."""
...
class BedAssignRequest(BaseModel):
    """Typed replacement for raw ``body: dict`` on bed assignment."""
...
class DutyToggleRequest(BaseModel):
    """Typed replacement for raw ``body: dict`` on duty toggle."""
...
class ClinicalNoteCreate(BaseModel):
    ...
    type: str = Field(
        "progress",
        pattern=r"^(observation|prescription|discharge-summary|progress)$",
    )
...
class ScheduleStatusUpdate(BaseModel):
    status: str = Field(
        ...,
        pattern=r"^(scheduled|completed|in-progress|cancelled|no-show)$",
    )


class ScheduleSlotCreate(BaseModel):
    """Typed replacement for raw ``data: dict`` on schedule creation."""
