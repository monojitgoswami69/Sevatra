"""User profile, addresses, emergency contacts, and medical conditions."""

from fastapi import APIRouter, Depends

from app.core.dependencies import get_current_user
from app.ambi.services.user_service import user_service
...
async def update_my_profile(data: ProfileUpdate, user: dict = Depends(get_current_user)):
...
@router.post("/me/medical-conditions", response_model=MedicalConditionResponse, status_code=201)
async def create_medical_condition(data: MedicalConditionCreate, user: dict = Depends(get_current_user)):
    return await user_service.create_medical_condition(user["id"], data)


@router.delete("/me/medical-conditions/{condition_id}")
async def delete_medical_condition(condition_id: str, user: dict = Depends(get_current_user)):
