...
Provides operations for the doctor dashboard: my patients, schedule,
clinical notes, and profile management.
"""
...
async def get_doctor_patients(db, staff_doc_id: str, hospital_id: str) -> list[dict]:
    """Return all admissions where the assigned doctor matches this staff member."""
    docs = (
        db.collection("life_admissions")
        .where("hospital_id", "==", hospital_id)
        .where("assigned_doctor_id", "==", staff_doc_id)
        .get()
    )
    results = []
    for d in docs:
...
async def create_schedule_slot(db, staff_doc_id: str, hospital_id: str, data: dict) -> dict:
    now = _now_iso()
    doc_data = {
...
async def update_schedule_status(db, schedule_id: str, staff_doc_id: str, new_status: str) -> dict:
    ...
    if data.get("doctor_id") != staff_doc_id:
        return None
    ref.update({"status": new_status, "updated_at": _now_iso()})
...
async def get_clinical_notes(
    ...
    query = db.collection("life_clinical_notes").where("doctor_id", "==", staff_doc_id)
    if patient_id:
...
async def add_clinical_note(db, staff_doc_id: str, doctor_name: str, data: dict) -> dict:
    now = _now_iso()
    doc_data = {
        "doctor_id": staff_doc_id,
...
async def update_doctor_profile(db, staff_doc_id: str, updates: dict) -> Optional[dict]:
    ref = db.collection("life_staff").document(staff_doc_id)
    doc = ref.get()
    if not doc.exists:
        return None
    ...
    allowed = {
        ...
        "contact", "email", "bio", "languages", "consultation_fee", "shift",
    }
