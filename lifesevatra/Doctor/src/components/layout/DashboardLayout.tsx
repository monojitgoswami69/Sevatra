        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center gap-3 px-6 py-6 border-b border-white/5">
            ...
            <div>
              ...
              <p className={`text-[10px] uppercase tracking-widest font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Doctor Portal</p>
            </div>
          </div>
            ...
            <div className={`rounded-xl p-4 mb-3 border ${isDarkMode ? 'bg-surface-darker/50 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="overflow-hidden min-w-0 flex-1">
                  <p className={`truncate text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{doctor.full_name}</p>
                  <p className="truncate text-xs text-slate-500">{doctor.specialty}</p>
                </div>
                <div className={`h-2.5 w-2.5 rounded-full flex-shrink-0 ${doctor.on_duty ? 'bg-primary shadow-[0_0_6px_rgba(19,236,19,0.6)]' : 'bg-slate-400'}`} title={doctor.on_duty ? 'On Duty' : 'Off Duty'}></div>
              </div>
            </div>
